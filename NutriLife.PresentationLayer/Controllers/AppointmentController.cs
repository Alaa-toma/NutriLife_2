...
namespace NutriLife.PresentationLayer.Controllers
{
    ...
    public class AppointmentController : ControllerBase
    {
        ...
        [HttpPut("ApproveAppointment/{AppointmentId}")]
        [Authorize(Roles = "Nutritionist")]
        public async Task<IActionResult> ApproveAppointment(int AppointmentId)
        {
            var result =  await _appointmentService.ApproveAppointmentAsync(AppointmentId);
            if(result == null)
            {
                return BadRequest(result);
            }
        ...
        {
            var result = await _appointmentService.RejectAppointmentAsync(AppointmentId);
            if (result == null)
            {
                return BadRequest(result);
            }
